...
use svg::{
    Document,
    node::element::{Path, Rectangle, path::{
...
fn convert(operations: &Vec<Operation>) -> Vec<Command> {
    ...
    for op in operations {
        ...
        path_data.push(Command::Move(Position::Absolute, (turtle.x, turtle.y).into()));
        turtle.wrap();
    }
...
fn generate_svg(path_data: Vec<Command>) -> Document {
    let background = Rectangle::new().set("x", 0).set("y", 0).set("with", WIDTH).set("height", HEIGHT).set("fill", "#fffff");
    let border = background.clone().set("fill-opacity", "0.0");
    let sketch = Path::new().set("fill", "none").set("stroke", "#2f2f2f").set("stroke-width", STROKE_WIDTH).set("stroke-opacity", "0.9").set("d", Data::from(path_data));

    let document = Document::new().set("viewBox", (0, 0, WIDTH, HEIGHT)).set("height", HEIGHT)
        .set("width", WIDTH)
        .set("style", "style=\"outline: 5px solid #800000;\"").add(background)
        .add(sketch)
        .add(border);
