...
use chrono::{DateTime, Duration as ChronoDuration, TimeZone, Timelike, Local, Utc};
use clap::{App, Arg};
use std::{mem::zeroed, net::UdpSocket, time::Duration, io::Error};
...
impl NTPMessage {
    ...
    fn parse_timestamp(&self, i: usize) -> Result<NTPTimestamp, Error> {
    ...
    fn rx_time(&self) -> Result<NTPTimestamp, Error> {
        self.parse_timestamp(32)
    }

    fn tx_time(&self) -> Result<NTPTimestamp, Error> {
...
fn ntp_roundtrip(host: &str, port: u16) -> Result<NTPResult, Error> {
...
fn check_time() -> Result<f64, Error> {
